...
class ImgFusion:
    ...
    def RF_Horizontal(self, img, D, sigma):
    ...
    def image_transpose(self, img):
        T = img.T
        return T


    def RF(self, img, sigma_s, sigma_r, joint_image, num_iterations=3):
        if np.shape(img)[0] != np.shape(joint_image)[0] or np.shape(img)[1] != np.shape(joint_image)[1]:
            ...
            for i in range(num_iterations - 1):
                ...
                F = ImgFusion.RF_Horizontal(self, F, dHdx, sigma_H_i)
                F = ImgFusion.image_transpose(self, F)
                F = ImgFusion.RF_Horizontal(self, F, dVdy, sigma_H_i)
                F = ImgFusion.image_transpose(self, F)
            F = F.astype(type(img))
            return F


    def dSiftFusion(self, img, imgRGB, scale, weighted_average='winner_take_all'):
        N, h, w = np.shape(img)
        img1 = img[:, 1:h, 1:w]
        imgRGB1 = imgRGB[:, 1:h, 1:w, :]
        dsifts = np.zeros((N, h - 1, w - 1, 32))
        extendimg = ImgFusion.imgextend(self, img, int(scale / 2 - 1))
        for i in range(N):
            extractor = DenseSIFT.DsiftExtractor(gridSpacing=1, patchSize=scale, nrml_thres=1.0, sigma_edge=1.0,
                                                 sift_thres=0.2)
